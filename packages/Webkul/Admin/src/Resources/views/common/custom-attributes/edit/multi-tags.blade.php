@if (isset($attribute))
    @php
        $lookUpEntityData = app('Webkul\Attribute\Repositories\AttributeRepository')
            ->getLookUpEntity($attribute->lookup_type, old($attribute->code) ?: $value,['id','name','color']);
    @endphp

    <multi-tags-component :attribute='@json($attribute)' :validations="'{{$validations}}'" :data='@json($lookUpEntityData)'></multi-tags-component>
@endif

@once
    @push('scripts')

        <script type="text/x-template" id="multi-tags-component-template">
            <div class="lookup-control">
                ...
                <div class="lookup-selected-options">
                    <span class="badge badge-sm badge-pill badge-secondary-outline" v-for='(result, index) in selected_results'
                        ...
                        <input type="hidden" name="tags[]" :value="result.id"/>
                        @{{ result.name }}
        ...
        <script>
            Vue.component('multi-tags-component', {

                template: '#multi-tags-component-template',
                ...
                data: function () {
                    return {
                        ...
                        search_route: this.searchRoute ?? `{{ route('admin.settings.attributes.tags') }}/${this.attribute.lookup_type}`,
                    }
                },
